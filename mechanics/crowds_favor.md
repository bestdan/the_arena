---
tags: [arena, mechanics, homebrew, social]
mechanic_type: social
complexity: moderate
status: active
visibility: public
created: 2025-11-21
updated: 2025-11-25
aliases: [favor, crowd, audience]
data_source: crowds_favor_options.yaml
---
# Crowd's Favor

While there are other ways to gain or lose favor, the most predictable is rolling with [Panache](panache.md). A successful `with Panache` roll will generally gain you one favor... a **failed** Panache roll with a natural result of 5 **or lower** (before modifiers) causes **–1 Favor**.

### 🔥 Hot Crowd 🔥

After **3 consecutive successful** Panache rolls, the crowd becomes **Hot** for that combatant:

- All further **with Panache** rolls gain **+2**.

- Hot status ends immediately when the combatant **loses any Favor**.

---

## Spending Crowd's Favor

| Cost | Name | Category | Action | Effect |
|------|------|----------|--------|--------|
| 1 | Crowd Distraction | control | reaction | When an enemy you can see begins moving, the crowd's shouts distract them. They must succeed on a Wisdom save or lose 10 feet of movement this turn as they look around confused. |
| 1 | Crowd Whispers | stealth | free | When an enemy makes a Perception check to find you, the crowd's murmurs create confusion. Impose disadvantage on their Perception check. |
| 1 | Damage Tax | defense | free | After dealing damage, regain 1d4 HP |
| 1 | Eyecatcher | support | free | Give an ally you can see +1 on their next attack roll or check |
| 1 | Flair Boost | offense | declared | Add +1d4 to any attack roll, skill check, or save (declared before or after rolling) |
| 1 | Flash Recuperation | defense | bonus | Gain temp HP = your proficiency bonus |
| 1 | Flash Step | mobility | free | Move 10 feet without provoking opportunity attacks |
| 1 | On Guard | defense | reaction | Gain +1 AC until the start of your next turn |
| 1 | Sprint Through Shadows | mobility | bonus | Until the end of your turn, your movement speed increases by 10 feet and you don't leave tracks or scent trails that can be followed by creatures tracking you. |
| 1 | Taunt | control | bonus | Force a creature within 15 ft. to make a Wisdom save or have disadvantage on its next attack against anyone but you |
| 1 | Up by your Bootstraps | mobility | bonus | Stand from prone using no movement |
| 2 | Bold Stance | defense | bonus | Until end of your next turn: can't be pushed or knocked prone; opportunity attacks against you at disadvantage |
| 2 | Crowd's Eyes | control | bonus | The scrying-watching crowd hints at nearby danger. Learn the direction and approximate distance (near/far) of the closest enemy to you. |
| 2 | Light in Their Eyes | defense | reaction | After an enemy hits you or an ally within 20 fie, force them to reroll the attack |
| 2 | Misdirection | control | bonus | Choose a creature within 30 ft. The crowd creates a distraction. The creature must make an Intelligence save or waste their action this turn as they react to false information from the crowd. |
| 2 | Moral Support | support | bonus | Take the Help action as a bonus action |
| 2 | Roaring Momentum | mobility | free | After hitting or succeeding, immediately move up to half your speed without provoking opportunity attacks |
| 2 | Salt in the Wound | offense | free | Add +1d6 to a single attack damage roll after you hit |
| 2 | Shadow Slip | stealth | free | After making a Stealth check, add +1d6 to the result. The crowd's attention shifts, creating deeper shadows around you. |
| 2 | Sizzle Sniper | offense | free | Your next melee or ranged attack ignores half cover and three-quarters cover |
| 2 | Stage Fright | control | bonus | Choose a creature within 30 ft. It must succeed on a Wisdom save or cannot take reactions until the start of your next turn |
| 2 | Swagger Step | mobility | free | Gain advantage on your next Acrobatics or Athletics check |
| 2 | The Chant Rises | defense | free | Gain advantage on your next saving throw |
| 2 | Turn the Knife | offense | free | Reroll damage you just rolled |
| 3 | Arena Acrobatics | mobility | bonus | Until the end of your turn, your movement doesn't provoke opportunity attacks and you can move through enemy spaces |
| 3 | Crowd Favorite | offense | free | Your next attack that hits deals an additional 2d6 damage |
| 3 | Crowd Shield | defense | reaction | When you would take damage, reduce it by 2d8 + your Charisma modifier (minimum 1) |
| 3 | Dramatic Dodge | defense | reaction | When targeted by an attack, impose disadvantage on the attack roll and move up to 10 feet without provoking opportunity attacks |
| 3 | False Name | control | free | When an enemy attempts to call you out or target you by name, you can cause the crowd to shout conflicting names. The enemy must make a Wisdom save. On a failure, their action is wasted as confusion overwhelms them. Usable once per encounter. |
| 3 | Mass Distraction | control | bonus | Choose up to 3 creatures within 30 ft. Each must succeed on a Wisdom save or have disadvantage on their next attack roll |
| 3 | Perfect Opening | offense | bonus | Your next attack roll before the end of your turn has advantage and scores a critical hit on a 19-20 |
| 3 | Rally the Arena | support | free | Grant +1d6 to all allies' next attack roll (once per round per arena rules) |
| 3 | Second Wind | defense | bonus | Regain HP equal to 1d10 + your proficiency bonus |
| 3 | Steal the Show | support | bonus | Grant an ally within 30 ft. advantage on their next attack roll and +1d8 to the damage if it hits |
| 3 | Tankin' Time | defense | bonus | Give yourself temporary HP = your proficiency bonus × 2 |
| 5 | Finisher's Spotlight | signature | declared | Declare at start of turn: your attacks deal +10 damage (flat) this turn |
| 5 | Turn the Tide | signature | free | Once per encounter: automatically succeed on a death saving throw and stand up with 1 HP |
| 6 | The Arena's Blessing | signature | free | For 3 rounds: all Panache rolls gain advantage; if you drop to 0 HP, drop to 1 instead (once) |