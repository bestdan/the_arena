---
tags: [arena, session, session-11, index]
session_number: 11
party_level: 5
status: ready
visibility: private
created: 2026-08-20
updated: 2026-08-20
---

# Session 11: Polymorphic Chaos

The magic colleges have bought the arena floor. Three crystal Fonts sing every third round, and whatever is standing near them stops being what it was. Four gladiators, one transmuter, and a hound whose bite changes flavour.

## Session Files

### For the GM
- [[session_11_setup_gm|GM Setup & Preparation]] — checklist, roleplaying hooks, combat flow, on-the-fly balancing
- [[session_11_encounter|Encounter Document]] — arena, Wild Shimmer rules, printable form tables, statblocks, round-by-round
- [[session_11_notes|Session Notes Template]] — Font trackers, transformation log, Favor and enemy status

### For the Players
- [[session_11_setup_players|Player Briefing]] — Snak's warning, the armory, the walk in
- [[../../items/snak_shop_session_11|Snak's Shop]]

## Quick Reference

**Pulse schedule:** end of rounds 3, 6, 9 — DC 14 Wisdom within 20 ft of a Font
**Fonts:** AC 15, 30 HP, immune poison/psychic; destroying one reverts transformed creatures within 15 ft
**Victory:** Magister Ilvane Corr defeated, yielded or fled, and the floor cleared
**The lever:** the Magister's *polymorph* is concentration; the Fonts' shimmer is not

**Source Material:** based on `ideas/polymorphic_chaos.md`, scaled from 4-6 PCs to 4 PCs at level 5
